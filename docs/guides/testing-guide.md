...
Configure a different W3C policy token once when constructing the application;
Otto applies it to routed responses, static files, and authentication failures:
...
at configuration time. Existing applications that pass `referrer-policy`
