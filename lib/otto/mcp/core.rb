...
class Otto
  module MCP
    ...
    module Core
      ...
      # :explicit scope. That scope accepts the canonical keys below, their
      # historical bare aliases (:endpoint, :validation, :rate_limiting), and
      # the Otto constructor spellings (:mcp_endpoint, :mcp_auth_tokens, ...).
      #
      ...
      # @option options [Boolean] :validation Enable JSON schema validation (default: true)
      # @option options [Boolean] :rate_limiting Enable rate limiting (default: true)
      ...
      # @example
      #   otto.enable_mcp!(endpoint: '/api/mcp', auth_tokens: ['secret'])
